...
def train_bnn_past_and_future(model, optimizer, dataloader, val_dataloader, batch_size, epochs, gamma, block, divide, kl_weight = 0.1, kl_loss = bnn.BKLLoss(reduction='mean', last_layer_only=False)):
    ...
    for epoch in range(epochs):
        ...
        torch.save(rank, 'rank_mc.pt')
        torch.save(var_sort, 'var_sort_mc.pt')
        torch.save(total_rl, 'total_rl_mc.pt')
        torch.save(total_estimate, 'total_estimate_mc.pt')
        torch.save(total_out_entropy, 'total_out_entropy_mc.pt')
        for i in rank:
        ...
        if epoch%50 == 0:
            torch.save(model.state_dict(), 'Para_DeepNet_MC') 